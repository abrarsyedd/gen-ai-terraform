...
module "keypair_f8bedb" {
  source                  = "./modules/keypair_f8bedb"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-keypair-f8bedb"
}

module "ec2_instance_f8bedb" {
  source                  = "./modules/ec2_f8bedb"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-ec2-f8bedb"
  instance_type           = "t3.micro"
  subnet_id               = ""
  vpc_subnets             = []
  vpc_security_group_ids  = []
  user_security_groups    = []
  encrypted               = false
  key_name                = module.keypair_f8bedb.key_name
  user_data = <<-EOF
  #!/bin/bash
  yum update -y
  yum install -y httpd
...
output "ec2_instance_id_f8bedb" { value = module.ec2_instance_f8bedb.instance_id }
output "ec2_public_ip_f8bedb"    { value = module.ec2_instance_f8bedb.public_ip }
output "ec2_private_ip_f8bedb"   { value = module.ec2_instance_f8bedb.private_ip }
