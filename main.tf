...
module "keypair_a8c65a" {
  source                  = "./modules/keypair_a8c65a"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-keypair-a8c65a"
}

module "ec2_instance_a8c65a" {
  source                  = "./modules/ec2_a8c65a"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-ec2-a8c65a"
  instance_type           = "t3.micro"
  subnet_id               = ""
  vpc_subnets             = []
  vpc_security_group_ids  = []
  user_security_groups    = []
  encrypted               = false
  key_name                = module.keypair_a8c65a.key_name
  user_data = <<-EOF
  #!/bin/bash
  yum update -y
  yum install -y httpd
...
output "ec2_instance_id_a8c65a" { value = module.ec2_instance_a8c65a.instance_id }
output "ec2_public_ip_a8c65a"     { value = module.ec2_instance_a8c65a.public_ip }
output "ec2_private_ip_a8c65a"    { value = module.ec2_instance_a8c65a.private_ip }
