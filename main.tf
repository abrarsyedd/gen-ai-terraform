...
module "keypair_e1c2ed" {
  source                  = "./modules/keypair_e1c2ed"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-keypair-e1c2ed"
}

module "ec2_instance_e1c2ed" {
  source                  = "./modules/ec2_e1c2ed"
  project_name            = "GenAI"
  environment             = "dev"
  name                    = "GenAI-ec2-e1c2ed"
  instance_type           = "t3.micro"
  subnet_id               = ""
  vpc_subnets             = []
  vpc_security_group_ids  = []
  user_security_groups    = []
  encrypted               = false
  key_name                = module.keypair_e1c2ed.key_name
  user_data = <<-EOF
  #!/bin/bash
  yum update -y
  yum install -y httpd
...
output "ec2_instance_id_e1c2ed" { value = module.ec2_instance_e1c2ed.instance_id }
output "ec2_public_ip_e1c2ed"     { value = module.ec2_instance_e1c2ed.public_ip }
output "ec2_private_ip_e1c2ed"    { value = module.ec2_instance_e1c2ed.private_ip }
